...
void log_line_speed_pause (float speed, int pause) {

	if (! log_file) {
		return;
	}
	fseek ( log_file , 0 , SEEK_SET );
	//fputs ( "1;000" , log_file );
	fprintf(log_file, "%3f, %d", speed, pause);
...
SCSAPI_VOID telemetry_frame_end(const scs_event_t UNUSED(event), const void *const UNUSED(event_info), const scs_context_t UNUSED(context))
{
	...
	log_line_speed_pause(telemetry.speed,output_paused);
	//log_print('\r');
	//log_print(
	//		//";%f;%f;%d;%d",
	//	"%f;%d\r",
